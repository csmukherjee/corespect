...
def MCPC_HDBSCAN(X, true_k, q=20,r=20, t=20, layer_ratio=None,scores=None,choose_min_obj=True,send_meta_data=False):
    ...
    for lbl in unique_labels:
        cluster_points = X_core[labels_hdb == lbl]
    ...
    final_labels[core_nodes] = labels_hdb
    ...
    node_to_rank = -1 * np.ones(n).astype(int)
    rank_counter = 0
    ...
    for rnd in range(num_step):
        ...
        final_labels, dist_to_centroids = pseudo_centroid_distances(final_labels, nodes_this_round,P_vec, ng_list, dist_to_centroids, node_to_rank)
    ...
    if send_meta_data:
        return final_labels, 0, centroids # 0 is for objetive value placeholder
